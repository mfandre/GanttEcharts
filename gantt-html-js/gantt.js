...
function renderAxisLabelItem(params, api) {
    ...
    if(isToDrawGroup == 1){ // group agrupator (Vertical rectangle)
        groupedElement.children.push({
            type: 'rect',
            shape: {x: 0, y: (params.coordSys.y-2*barHeight)-(barHeight/3), width: 10, height: 46},
            style: {
                fill: groupColor,
            }
        })
...
function mapZebra(taskData){   
    ...
    for(let index = 0; index < taskData.length; index++){    
        let item = taskData[index]
        let index_attributes = [index, getMinDate(taskData), getMaxDate(taskData), item.taskId];
        mappedData.push(index_attributes);
    }
    return mappedData
}
